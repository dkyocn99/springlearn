...
@RestController
public class DimmyControllerTest {
...
//        Page <User> users = userRepository.findAll(pageable);
    ...
    public User detail(@PathVariable int id) {
        ...
        //user/4찾으면 내가 데이터베이스에서 못찾아오게 되면 user가 null이 될 거 아냐?
        //그럼 return null이 리턴 되자나 그럼 프로그램에 문제가 있지 않을까?
        //Optional로 너의 User객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return해!!
        //.get -> 그럴 일 없어!  .orElseGet  .orElseThrow
...
//UPDATE
// email. password
    //save함수는 id를 전달하지 않으면 insert를 해주고 id를 전달하면 해당 id에 대한 데이터가 있으면 update를 해주고 해당 id데이터가 없으면 insert를 한다.

    @Transactional //save를 하지 않아도 된다.
    //더티 채킹
    @PutMapping("/dummy/user/{id}")
    public User updateUser(@PathVariable int id, @RequestBody User requestUser) { //json 데이터를 요청 => Java Object(MessageConvert의 Jackson라이브러리가 변환해서 받아줘요.)
